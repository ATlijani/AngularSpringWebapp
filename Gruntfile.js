module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    copy: {
      tomcat: {
        ...
        dest: '<%= folders.webapp.build %>WEB-INF/web.xml'
      },
      resources: {
        expand: true,
    ...
    watch: {
	  tomcat: {
		files:['<%= folders.webapp.root %>web.xml'],
		tasks: [ 'copy:tomcat' ]
	  },
	  resources: {
  ...
  grunt.registerTask('dev', ['bower_concat', 'copy', 'concat']);
  grunt.registerTask('dist', ['jshint', 'bower_concat', 'copy', 'concat', 'uglify']);
